...
import { agentLoopPrompt, componentKey, evalExpect, flowSkill, genArtifactUi, genComponent, goalStatus, matchComponent, neededCredentials, parseAgentStep, plan as shenronPlan, renderPlan, runGenEval, toLangflowFlow, visibleTo } from './shenron.mjs';
...
async function runAgentSync(name, input, { run = runVendorAsync, dispatch = mcpDispatch, depth = 0 } = {}) {
  ...
  if (!tools.length) {   // 道具なし → 従来通り単発（後方互換・既存 agent は不変）
    ...
    const result = await run(vendor, prompt, lc.stub, { model });
  ...
  for (let i = 0; i <= MAX; i++) {
    ...
    const out = await run(vendor, convo + hint, lc.stub, { model });
