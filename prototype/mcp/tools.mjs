...
export const TOOLS = [
  ...
  { name: 'test_notify', description: '有効な通知 integration (kind:notify) 全てにテスト payload を送信し、各 URL の成否を返す。',
    inputSchema: { type: 'object', properties: {} } },
  { name: 'create_agent', description: 'エージェントを作成 → 即この MCP の tool 一覧に agent_<name> として現れ、すぐ呼べる。name は小文字[a-z0-9-]。instructions=システムプロンプト（役割・出力形式）。vendor 既定 claude（あなたのサブスク＝従量0）。tools を渡すと「道具使用エージェント」になり、許可された read-only ツール(recall/list_*/get_*/hub_health 等)や他 agent(agent_<name>)を有界ループで呼んで答える（side-effect ツールは安全のため不可）。',
    inputSchema: { type: 'object', properties: { name: { type: 'string' }, instructions: { type: 'string', description: 'システムプロンプト（役割・振る舞い・出力形式）' }, vendor: { type: 'string', description: 'claude（既定）/codex/gemini/ollama' }, model: { type: 'string' }, tools: { type: 'array', items: { type: 'string' }, description: 'P-4: 使える read-only ツール名 or agent_<name> の allowlist（例 ["recall","list_goals","agent_foo"]）。省略で従来の単発プロンプト。' }, skill: { type: 'string' }, stub: { type: 'string', description: 'vendor 不在時のフォールバック出力' }, accepts: { type: 'array', items: { type: 'string' } }, emits: { type: 'array', items: { type: 'string' } }, company: { type: 'string' } }, required: ['name', 'instructions'] } },
