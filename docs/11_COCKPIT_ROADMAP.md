...
> 更新: 2026-06-16（**Phase 1＋Phase 2（H★/I/J）完了**＝Trust Boundary・cross-vendor consensus・build-state IR を実機検証。UI/UX も Langflow 実機参考に整理）
...
- **Wave H — Agent Trust Boundary（★wedge・最重要差別化）✅ DONE（最小・実機検証）**：zero-dep **`prototype/trust.mjs`**（`redact`＝secret/PII/env パターン＋per-agent `never` を除去・値は残さず what だけ記録／hash-chain `auditAppend`/`auditVerify`／`capability passport`）。hub が **毎ホップ強制**：(1) `create()`＝handoff 入力を data firewall に通す（recipient passport の never＋既定 secret）→ 値は recipient に届く前に `[redacted:label]`、(2) `runMcp()`＝**外部送信前に capability `external_send` を要求**（上流 agent が無ければ deny）＋ egress で再 redact、(3) `approve`/`redact`/`deny`/`send`/`passport` を **改ざん不能 trail** に。`/api/audit`（+verify）・`/api/agents/:id/passport`。cockpit＝agent inspector に passport 編集（caps チェック＋never）・inbox に 🔒redacted バッジ・🔒監査トレイル＋verify バッジ。**done（達成・全 API 実機）**: cross-boundary handoff で email/openai-key/env-secret＋codename を除去（**file path は保持**）／external_send 剥奪 agent→mcp は **deny＋outbox 空**／audit chain verify ok／`inbox.json` を手で改ざん→再起動→**verify=ok:false at:0**（tamper 検出）。＝**n8n/Langflow/Zapier に書けない flow**（単一オーナー前提を捨てた trust）。🟡 残: pass-allowlist は構造化 payload のみ・真の cross-party 認可（OBO/DPoP・M5）は別軸 North Star（混同しない）。
- **Wave I — cross-vendor consensus node（vs vendor-native）✅ DONE（最小・実機検証）**：`kind:"consensus"` component。hub `fireConsensusNode`/`runConsensus` が同 task を **N vendor に並列 fan-out**（`runVendorAsync`・`Promise.all`）→ **token-Jaccard medoid**（他と最も似た出力）を選び **agreement スコア**算出 → 下流へ。内部 compute ゆえ approval フェンス無し・handoff backed（可視化＋crash-resume）。cockpit＝palette に「⚖️ Consensus」（vendors/prompt の typed field）、ノードに `picked · agree X`。**done（達成）**: Chat Input→Consensus(claude,codex,gemini)→Chat Output を実 Run、3 vendor 並列・picked=claude・agreement=0.75・consensus header 付き出力が下流へ（3/3 completed）。**単一 vendor は構造的に不可能**＝「なぜ Claude native でなく BuildHUD?」の構造回答。🟡 stub mode は vendor-tag 違いで agreement 高め（機構実証）／真の多様性は実 vendor 時。
- **Wave J — build-state IR（vs iPaaS）✅ DONE（最小・実機検証）**：trigger 語彙を第一級化＋ no-eval match DSL（n8n の generic webhook と差＝「IR 深いほど堀」§4）。**IR 語彙（10）**: `pr_opened`/`pr_merged`/`review_completed`{status:green|red|changes_requested}/`ci_passed`/`test_red`/`rc_built`/`deploy_green`/`deploy_failed`/`issue_filed`/`release_tagged`（各 fields 定義）。**match 演算子（8・no eval）**: `$in`/`$nin`/`$ne`/`$gt`/`$gte`/`$lt`/`$lte`/`$exists`（leaf が全 key `$` 始まりの時に発火・それ以外は従来 deep-subset）。hub `GET /api/buildstate` で IR を公開、`deepMatch` を拡張、**`mcp/server.mjs` も同 semantics に mirror**（fire_event 共有）。cockpit＝trigger inspector に **event ドロップダウン**（語彙＋fields 表示・選ぶと match 自動生成、JSON 編集で演算子も可）。**done（達成）**: `review_completed` + `status:{$in:[green,passed]}` の automation が green/passed で発火・red で不発火を `/api/fire` で実機検証／IR スキーマを本節に doc 化。
...
- **Wave L — Ghost Writer（NL→flow 著述 meta-agent）✅ DONE（最小）**：実装＝hub `/api/ghostwrite {prompt}`。**heuristic builder**（vendor=stub/offline 既定）が index を keyword 照合し **Chat Input/trigger →（一致 agent を言及順に連鎖）→ mcp 送信?→ Chat Output** を構築、event 駆動文（PR merge/deploy/review）は **trigger を入口**に。一致 agent 無し→ **Prompt step ＋ 新規 agent draft**（name/skill/accepts/emits/systemPrompt）。real vendor 時は **LLM が flow JSON 生成**（同経路で検証）。全 edge を **typed-port 検証**（不一致は drop＋warning）＋ **auto-layout**（longest-path）。cockpit「✨ Ghost Writer」→ prompt → `loadFlow` で canvas に materialize（agent は HIDDEN 解除・component/mcp/trigger 配置・edge 復元）→ human レビュー → ▶Run（**生成≠実行**・approval フェンス維持）。draft agent は `/api/agents` で **runnable な local agent** 登録可。**done（達成）**: 「find prospects then draft outreach and send email」→ input→sales→marketing→echo.send_email→output（0 warn）／event 文→trigger 入口／一致無し→prompt＋draft／生成 flow を実 Run で **4/4 completed** 検証。⚠️ **Langflow Assistant（v1.9/1.10）が既に NL→完全 flow** → 「flow 生成」だけなら劣後。差別化は **(a) cross-owner agent を含めて組む ＋ (b) Wave H の capability passport 付与＝fenced 著述**（H とセットで初めて価値）。本最小版は (a) の素地（cross-company agent も index 経由で配線可）まで。
...
- **Wave E3 — run 後の verdict（Trust Summary）✅**（commit `b80a117`）: 既定 inspector 最上部に **最新 run の verdict ブロック**＝`trustSummary()` が tamper-evident audit ＋ run state を join（新 endpoint 無し・`/api/audit`＋`/api/state` で完結）。表示＝除去数＋leak chips（firewall が実際に何を取ったか）・external send の sent/held/blocked・router 分岐決定・skipped 枝数・chain verify バッジ（ok=緑/改ざん=赤）。＝raw audit を「証拠」、verdict を「答え」に昇格。**検証**: secret→router→agent→承認制 send の flow で 除去 2(openai-key×1,env-secret×1)・sent 0・held 1・skipped 1・route r1 redacted=true→then・verify ok（実 audit/handoff state と一致）。files: `ui.html`(trustSummary)。**＝E1→E2→E3 トリロジー完了**（実行前に安全が見える→安全に分岐→走った後に証明）。
