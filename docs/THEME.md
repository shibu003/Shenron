...
1. **フラット（発色なし）** — glow / neon（`box-shadow` の光彩・`text-shadow`）は使わない。攻殻感は色ではなく**視覚的な構造モチーフ**（等幅フォント・`›` プロンプト・照準レティクル）で出す。**用語/ラベルは攻殻化しない**（下記「用語」）。
...
| `firewall` | ファイアウォール（🔒） | `M12 3l8 3v5c0 5-3.4 8.2-8 10-4.6-1.8-8-5-8-10V6z` ＋ `M9.5 12l1.8 1.8L15 10`（盾＋✓） |
...
ラベル・i18n テキストは**機能名のまま**。攻性防壁 / ネットダイブ / タチコマ / 公安9課 等の言い換えは**しない**（user 決定 2026-06-24）。

- firewall は「ファイアウォール」、discover は「発見」、component は「コンポーネント」、control plane は「コントロールプレーン」のように、機能が一目で分かる語を使う。
- 攻殻らしさは**視覚（色・アイコン・フォント・レティクル）だけ**で出す。テーマのために意味の分かりにくいジャーゴンを入れない（[[feedback_human_labels_not_jargon]] と整合）。
- ブランド名 `神龍` はプロダクト名なので維持（攻殻用語ではない）。
...
- **用語の攻殻化**（攻性防壁/ネットダイブ/タチコマ 等のラベル化）。機能名のまま。
