...
- ✅ **cockpit UI**（c8457a1）: `ui.html` に 🐉 神龍 ボタン → goal モーダル → `/api/shenron/plan` → `loadFlow` 描画（gap 数＋Wave2 suggestion を conn/toast）。plan 後に 🔗 Langflow化 ボタン出現 → `/api/shenron/build` → `importLangflowFlow` で round-trip（browser smoke: plan 3 nodes → build で 🔗 ゼロ・count 一致確認）。`ghostWrite()` 雛形に複写。**注**: 🐉 神龍 と ✨ Ghost Writer が両方 NL→flow（神龍が上位互換：gap 解決＋外部探索）。Ghost Writer は agentDraft 分岐を持つので即削除せず保留＝要 user 判断（fold/remove?）。
