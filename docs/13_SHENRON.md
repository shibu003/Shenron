...
**残る Langflow の紐（3 本）**: ① `toLangflowFlow`/`importLangflowFlow`＝**interop（他人の flow の on-ramp）として残す**（依存でなく機能）／② exotic node の `langflowRun` 委譲 → **生成 MCP server 実行に置換（Wave 9 ✅: 生成→integration 登録→`mcp` node で走る）**／③ model fidelity → **model API 直叩きで native 化**。
...
→ **§J Wave 9 ✅**: runnable 組込は「生成 MCP server を integration 登録→既存 `mcp` node で走らせる」で実装（Langflow custom_component でも第2 python-runtime でもない）。検証＝prod の stdio client で spawn+handshake（prod パス＝検証パス）・approval gate で人ゲート・`safeEnv` で生成コードから secret を抜く。
...
> 📍 **wave の一覧・状態の正本は `docs/ROADMAP.md`**。本節は各 Wave の詳細実装ログ。
...
- **Wave 8 残 → Wave 9 で解消**: 旧「native python-component で走らせる(8a)」は **Wave 9 に吸収済**（生成 server を integration 登録→既存 `mcp` node で走らせる＝独立/ladder と整合・第2 runtime を作らない）。意味的 dedup・部品の編集/削除・Wave 2 外部 registry 連結・OS サンドボックスは引き続き残課題。
- ✅ **Wave 9（生成ターゲットを Langflow Component → standalone MCP server に転換・8a 吸収・閉ループ完成）**: 北極星「Langflow 独立」を生成層に反映。利点3点＝① `mcp` node が消費＝Langflow runtime 不要 ② 検証が prod 同形（prod の stdio client で spawn）＝stub-gap 消滅 ③ 一度生成した server が ladder 第1段に rejoin（integration 登録→再生成不要）。
  - **secret-env fence（load-bearing）**: `mcp-client.mjs` に `safeEnv(extra)`＋`SECRET_RE`（strip 正規表現の single source）。`callStdio(...,{cwd,env})`＝env を渡した時だけ `spawn` に適用（**未指定なら process.env 継承＝信頼済 server 不変**）。生成コードは untrusted ゆえ verify/run 両方で credential を抜いて実行。
  - **生成→MCP server**: `GEN_PROMPT`/`REPAIR_PROMPT` を stdlib-only Python MCP server（JSON-RPC 2.0 over stdio・1 tool `run`・`def run(input=None,**kw)`＝optional input/unknown 引数無視で prod skew 対策・`print(json.dumps(msg),flush=True)`・stdout は JSON-RPC 専用/log は stderr）に書換。`verifyMcpServer(code)` が `runInSandbox`+`HARNESS` を**置換**＝disposable dir に `server.py` 書出→`callMcpTool({command:'python3 server.py'},'run',{input:''},{cwd,env:safeEnv()})` で **spawn+handshake+run** 検証（**prod パス＝検証パス**）→非空 text なら ok、isError/timeout/spawn 失敗は tail を error に。`genComponent` の sandbox default を `verifyMcpServer` に（制御フロー・返り shape 不変・`await sandbox(code)` に）。
  - **ladder rejoin**: approve route（hub.mjs）拡張＝`approveComponent`→`prototype/mcp/generated/<id>.py` 書出＋`saveIntegration({id,kind:'mcp',command:'python3 prototype/mcp/generated/<id>.py',generated:true,tools:[{name:'run'}]})`（command は REPO_ROOT 相対＝`runMcp` が `cwd:REPO_ROOT` で spawn・echo と同形）。`saveIntegration` に `generated` flag を通す。
  - **prod-run fence＋vetted 廃止**: `runMcp` が `integ.generated` なら `callMcpTool(...,{cwd:REPO_ROOT,env:safeEnv()})`（run 時も secret strip）。`buildPlanIR` の `vetted` placeholder と `components` 引数を**削除**＝approved→実 integration の `run` tool が planner inventory（hub.mjs が `it.tools`→`<id>.run` flatten）に出る→LLM が `mcp:<id>.run` に解決＝placeholder に届かない。plan route の `components` inventory 二重注入も削除（matchComponent は gen-component の cache 照合用に維持）。
  - **UI**: `shenronGaps` が converged 後に **「✅ 承認して登録」**（`/api/shenron/components/approve {id}`→`/api/shenron/plan {goal,save}`→`applyPlan` で canvas が gap を実 `mcp:<id>.run` node に再解決）。i18n `sh_gaps_*`＋`sh_approve`/`sh_approved` を MCP server 文言に。
  - **BYO-credential 注入（Wave 9.1・「認証付き API」穴埋め）**: `safeEnv` の default-deny は維持しつつ、**承認済み生成 server が宣言した env 名だけ**を Run 時に戻す（per-component allowlist）。①`mcp-client.mjs` `pickEnv(names)`＝allowlist から process.env の値を引く（single source）。②`shenron.mjs` `neededCredentials(code)`＝コードが読む env 名のうち `SECRET_RE` 一致を抽出（pure）。`GEN/REPAIR_PROMPT` は「認証要なら `os.environ.get('NAME')`・無ければ env 名を明示・hard-code 禁止」に緩和。`verifyMcpServer(code,{creds})`/`genComponent` は cred を抽出→**env に揃えば `safeEnv(pickEnv(creds))` で live verify／欠けていれば `needsCredentials` を surface（repair 空回りを止め、operator が key を hub env に入れて再生成する導線）**。③`hub.mjs` `saveComponent`/`saveIntegration` が `credentials:[names]` を保存（**値は持たない**）、approve が allowlist を載せ、`runMcp` が `safeEnv(pickEnv(integ.credentials))` で**宣言名だけ**注入＋`trail('credential-inject',{names})`（**名前のみ・値は出さない**）。④UI=`sh_gaps_need_cred`＋「env (Run 前に hub に設定): X」表示。**secret は repo に乗らない**（`integrations.json` は tracked＝名前のみ・値は operator の env/`.dev.vars`＝gitignore 済）。🔴 **正直**: フェンスは「キー無し」→「人が承認した narrow allowlist」に弱化（approved-but-malicious server は宣言した1キーを exfil し得る）→緩和＝human-gate のコード目視＋宣言名のみ、egress allowlist は v2。
  - **検証**: `test_shenron.mjs` に ① `verifyMcpServer` を real python3 fixture で round-trip（good→`{ok,実データ}`/即 exit→`{ok:false,error}`）② **secret-fence**（生成 server が `os.environ['FAKE_API_KEY']` を読もうとしても空・`HOME` は残る）③ buildPlanIR が `mcp:cmp-x.run` を実 mcp node に解決＝missing にしない／未解決 gap は ⚠️ placeholder ④ **BYO-credential**（`neededCredentials` scanner／genComponent の missing-cred surface＋allowlist threading／verify で allowlisted key 注入・非 allowlist は strip）。既存 genComponent repair-loop（fake sandbox）維持。**残（v2）**: 実 claude e2e（live hub で gap→生成→承認→再 plan→runflow 実データ）・OS サンドボックス（seccomp/egress allowlist）・「キー無しでもコード目視で approve」UI・no-API gap→computer-use（Wave 11）。

### 「まだ MCP に無いもの」決定木（更新版・Wave 9.1 反映）
> 「API を登録していない人」をどう救うか＝gap の素性で 4 分岐:
> - **公開 API あり（no-auth）** → MCP server 生成（Wave 9 ✅）。**登録不要**＝神龍が裏で stdlib server を作って叩く。「API 未登録の人」の大半はここで無自覚に解決。
> - **API あり＋認証(key) 要** → MCP server 生成＋**BYO-credential 注入（Wave 9.1 ✅）**。生成 server が宣言した env 名だけ Run 時に注入。**operator が hub の env に key を置けば**動く（値は repo に乗らない）。key を持たない/置けない人はこの枝で止まる→下の no-API 枝（computer-use）に逃がす。
> - **API 無し（UI のみ）** → **computer-use（Wave 11・未実装）**＝API 登録不要。ユーザーは普通にブラウザで**ログインするだけ**、agent がその session に乗って UI 操作（concierge の「一緒に操作」）。key の代わりに人のログイン session を使う。送信系は always-human（ToS 線）。
> - **ただの変換** → prompt（built-in・登録不要）。

**case「no-API / key 無し」の施策（Wave 11 へ繋ぐ）**: ①短期＝該当 gap を `agent`(browser-control) node に解決し human-in-loop で操作（11c）。②鍵の所在＝API key を「登録」する代わりに**ブラウザの既存ログインを資産**にする（Cookie/session を worker が保持・11a の stateful client）。③ToS 生存条件＝outbound（送信/購入/応募）は **always-human**＝AI 下書き→人が押す（11b checkpoint）。→ つまり「API 未登録でも UI さえあれば概ね到達」できるのが北極星。詳細は下の Wave 11。

### 次の Wave（10/11・設計確定・未実装／plan: `~/.claude/plans/reflective-sparking-pearl.md`）
> 2026-06-19 plan 承認済。生成ターゲット＝standalone MCP server は **Wave 9 で実装済**・BYO-credential は **Wave 9.1 で実装済**。

- **Wave 10 — gate UI（8b・残）**: 再設計後 ui.html（1f84043: topbar/`#viewnav`/`#view-runs`・`viewSwitch`:543・`openModal`:1716・`api`:665）。①`showFlows`(:1301) flowcard に **📜 skill button**（Wave 7 で defer・`POST /api/shenron/skill`）。②`shenronGaps` の **多 gap 巡回**（Wave 9 で承認+re-plan は実装済・残=gap[0] 固定→`LAST_PLAN.missing` 巡回）。③**pending 部品パネル**（`#view-runs` or viewnav 5th・`GET /api/shenron/components`→⚠️N pending→承認）。検証=browser smoke（ui.html は unit test 無し・edit 前に再 Read）。
- **Wave 11 — computer-use agent node（北極星本丸・最大）**: **構造的発見＝`mcp` node は stateless 一発で session 保てない→Playwright 多段は stateful agent worker 必須**。3 サブ: **11a** 新規 `prototype/agents/browser-worker.mjs`（`worker.mjs` テンプレ・`/api/poll {agent:'browser-control'}`・Playwright MCP への **持続 client session を worker 内に保持**・各 action `trail`+`redact`）。**11b** co-pilot step-pause＝**新 hub primitive**（worker が submit 直前 `POST /api/handoffs/:id/checkpoint {screenshot}`→`awaiting_approval`→人 approve/直し→`/resume`。現 approval は handoff 単位＝新 endpoint 要・着手前に独自 Explore+Plan）＋**ToS guardrail＝送信系は always-human（auto 不可）**。**11c** planner 統合（no-API gap→`agent`(browser-control) node 解決）。リスク: 🔴 computer-use の brittle さ＝1 サービス・人 loop 内に固定。
- **推奨順序 Wave 10→11**（9=閉ループ基盤 ✅ 出荷済・10=cockpit 完成・11=別設計パス）。各 revertable commit 境界。
