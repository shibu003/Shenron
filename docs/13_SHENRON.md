...
- **Wave 8 残 → Wave 9 に再構成**: 旧「native python-component で走らせる(8a)」は **Wave 9 に吸収**（生成 server を integration 登録→既存 `mcp` node で走らせる方が独立/ladder と整合・第2 runtime を作らない）。意味的 dedup・部品の編集/削除・Wave 2 外部 registry 連結・OS サンドボックスは引き続き残課題。

### 次の Wave（9/10/11・設計確定・未実装／plan: `~/.claude/plans/reflective-sparking-pearl.md`）
> 2026-06-19 plan 承認済。**生成ターゲットを Langflow Component → standalone MCP server に転換**（§0/§1 北極星: Langflow 独立／理由3点＝`mcp` node が消費で Langflow 不要・検証が prod 同形で stub-gap 消滅・一度生成した server が ladder 第1段に rejoin）。「まだ MCP に無いもの」決定木＝**API あり→MCP server 生成／API 無し(UI のみ)→computer-use(手)／ただの変換→prompt**。**8a（native python-exec）は Wave 9 に吸収**（独立 Wave にしない）。

- **Wave 9 — 生成ターゲットを MCP server に（最優先・8a 吸収）**: 6 commit。①`mcp-client.mjs` に `safeEnv()`（runInSandbox の secret-strip 正規表現を single source 化）＋`callStdio` に `env` オプション（未指定なら従来通り process.env 継承＝信頼済 server 不変）＝**生成コード untrusted の secret-env fence（load-bearing）**。②`GEN_PROMPT`/`REPAIR_PROMPT` を stdlib-only Python MCP server（JSON-RPC over stdio・1 tool `run`・`def run(input=None,**kw)`・`flush=True`・stdout は JSON-RPC 専用）に書換、`verifyMcpServer(code)` が `runInSandbox`+`HARNESS` を置換＝disposable dir に server.py→**`callMcpTool({command:'python3 server.py'},'run',{input:''},{cwd,env:safeEnv()})` で spawn+handshake 検証**（prod パス＝検証パス）。`genComponent` 制御フロー不変。③approve route（hub.mjs:691）拡張＝`prototype/mcp/generated/<id>.py` 書出＋`saveIntegration({kind:'mcp',command:'python3 ...',generated:true,tools:[{name:'run'}]})`＝**ladder rejoin**。④`runMcp` が `generated` integ を `safeEnv()` で実行＋`buildPlanIR` の `vetted` placeholder **削除**（approved→実 integration の `run` tool が inventory に出る＝LLM が `mcp:<id>.run` 解決）。⑤UI: `shenronGaps` に「承認して登録」＋re-plan。⑥docs。リスク: verify/prod arg skew(`{input:''}`)・stdout block-buffer(`flush=True`)・repair signal は server が traceback を `isError:true` text に。e2e: 「fetch BTC price」gap→生成→handshake 検証→承認→再 plan が実 `mcp:<id>.run` に→runflow で実データ＋secret-fence(`FAKE_API_KEY` が見えない)。
- **Wave 10 — gate UI（8b）**: 再設計後 ui.html（1f84043: topbar/`#viewnav`/`#view-runs`・`viewSwitch`:543・`openModal`:1716・`api`:665）。①`showFlows`(:1301) flowcard に **📜 skill button**（Wave 7 で defer・`POST /api/shenron/skill`）。②`shenronGaps`(:1645) を **承認+re-plan＋多 gap 巡回**＋i18n `sh_gaps_*`(:469) を MCP server 文言に。③**pending 部品パネル**（`#view-runs` or viewnav 5th・`GET /api/shenron/components`→⚠️N pending→承認）。検証=browser smoke（ui.html は unit test 無し・edit 前に再 Read）。
...
- **推奨順序 Wave 9→10→11**（9=閉ループ基盤・10=cockpit 完成・11=別設計パス）。各 revertable commit 境界。
