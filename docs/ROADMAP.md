...
**✅実装済み（`e840690`・未 push）**：`runner.mjs` `runVendorAsync` に `vendor==='mock'` 分岐（env `SHENRON_MOCK_PLANNER` の JSON 配列を 1 コールずつ shift・文字列=生出力／オブジェクト=stringify）＋ `test_all.mjs`（glob 子プロセス集計）＋ `test_planflow_http.mjs`（clarify→clarify→clarify(再回答)→plan の 4 ターン HTTP）＋ **#2 dedup-by-question**（`mergeBrief` の confirmed を質問キーで最新上書き＝`briefItems(_, byQuestion=true)`・assumptions/blockers は全文 dedup のまま不変）。**`node prototype/hub/test_all.mjs` で 13/13 green**（既存 12＝予測 11 より +1・`test_routes.mjs` が R2 B8 で既存化していた／glob runner ゆえ count 非依存で自動追従）。手動実機起動（`--vendor mock --port 8802`）で health/readiness/plan 3 連も確認。
