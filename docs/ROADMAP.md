...
| **Wave Cockpit（玄関統合 + ノード検証 + UI/UX）✅0/1/2** | 3 cockpit(ui.html旧/ui2作業場/shenron事務所)の drift を「玄関 router」で統合。**`Cockpit-0`(検証)/`1`(玄関+ui.html退役)/`2`(攻殻 UI テーマ統一+⌘Kナビ+役割分離) は完了・PR #1 `50acb65` で main マージ済**。**`3`(UX 磨き)** = docs/COCKPIT-3-UX-IMPROVEMENTS.md 設計済・spacing/input/scrollbar/animation/transition 統一化・1 commit で ui2/shenron/settings の CSS 磨き。色/HTML/機能は不変。B1 の前後可。 | ↓「Wave Cockpit 計画」 |
...
| **Wave P-4 tool 使用エージェント** | Agent Factory のエージェントを**単発プロンプト完結 → tool 使用エージェント**化。`create_agent` に `tools`(read-only allowlist) を渡すと `runAgentSync` が**有界 ReAct ループ**（LLM が `{tool,args}`/`{answer}` を JSON 出力→`parseAgentStep`→許可ツールのみ `mcpDispatch`/sub-agent 実行→結果を `redact()` してから vendor 再注入・MAX4step）。tools 省略は従来の単発（後方互換）。⚠️**承認フェンス不変**: `SAFE_AGENT_TOOLS`=read-only のみ（set_*/run_*/gen_*/credential/承認系を除外）→ side-effect は次スライス(非同期 checkpoint)。sub-agent 再帰は depth<2 で有界化。`create_agent` に `tools/skill/stub/accepts/emits/company` 露出（hub `createAgent` との fidelity gap 解消）。MCP 両surface・test 緑(surface guard 71/59)。 | 本 doc |
| **Wave UI S6 — S3 配線** | ui2.html が `get_flow_ui`(GET /api/workflows/:id/ui) を消費し **flow 紐付け成果物 UI を自動表示**。成果物パネルに native `<select>`（`hasUi` flow を列挙）→選択で保存済み JSX を取得→`renderArtifact`。S3 backend 出荷済だが cockpit 未消費だったループを閉じる。backend 変更ゼロ・inline JS 構文緑。 | 本 doc |
...
**✅ 実装結果（2026-06-25・`21dee59`）**：`resolveVendor({explicit,tier,fallback})→{vendor,model}`（tierRoute 直後に定義）に集約。優先順位 `explicit(node 明示) > tier route > EXEC_VENDOR > fallback(agent 既定) > 'stub'` をバイト等価で保存。集約サイト＝`runLocal`(handoff)/`runPrompt`/escalate(strong)/`runAgentSync` の4箇所。⚠ **Pass2 で2つの非対称を発見・設計の「単一優先順位」前提を修正**：① **consensus(`EXEC_VENDOR||v`)は EXEC_VENDOR 最優先の逆契約**＝統一すると `--vendor stub` の全 vendor 強制が壊れるゆえ**対象外で残置**。② **model 解決が vendor と独立**＝runLocal handoff は vendor で `lc.vendor` を fallback にするが model は `h.model` 直（`lc.model` を使わない）→ `fallback` の vendor/model を**独立引数**にして吸収。生成系(`EXEC_VENDOR||'claude'` 6箇所)・judge(`tierRoute('cheap')` 直渡し)も既定値が違うゆえ対象外。検証＝hub test 11本 green（test_nodes＝vendor 主オラクル）＋runner 契約 test＋hub 実起動 `/api/health` OK。
...
**✅ 実装結果（2026-06-25）**：4 生成サイト（agent `create`／firePromptNode／fireConsensusNode／fireMcpNode）に `kind:'agent'|'prompt'|'consensus'|'mcp'` を**追加**（既存 marker `h.prompt`/`h.consensus`/`h.mcp` は payload 運搬役として温存＝表示/payload 不変）。型判別を**純関数 `hkind(h)=h.kind||(marker fallback)`** に集約し、判定サイト＝`sweep`(recovery)＋`approve` の2箇所が呼ぶ。⚠ ROADMAP の「`h.kind ||=` で old data を mutate する migration shim」は**読取専用 fallback に変更**＝write-on-read を避け副作用ゼロで「旧 inbox.json 安全」を達成（marker fallback がその機構）。payload 使用6箇所（runPrompt/runConsensus/runMcp/API response）は不変。検証＝test 11本 green（`test_reliable`＝crash recovery 主）＋hub 実起動で新 handoff に `kind:'prompt'` 付与・旧 handoff(`<none>`)併存を実証・`/api/health` OK。
...
**✅ 実装結果（2026-06-25）**：`fireNode` の 11連 if（旧 L536-557）を単一 `const RUN`（kind→handler）に集約し、本体を `(RUN[node.kind]||RUN.__agent)(run,node,input,from)` の1行へ。委譲系（prompt/consensus/router/mcp/workflow）は `fireXNode` を**直値マップ**（wrapper arrow ゼロ）、inline 系（input/output/parser）と変換系（languagemodel/structured/model）だけ arrow で表内包。fallthrough は `RUN.__agent`（外部 agent handoff）。**model は config.mode 分岐を arrow body に保持**＝`...node.config` spread（languagemodel/structured kind 版には無い非対称）を1ビットも変えず移送し byte 等価を維持。⚠ **hoisting 安全性を事前確認**＝`RUN` 初期化子が値参照する5 `fireXNode` は全て `function` 宣言（完全巻き上げ）ゆえ textual に後方でも TDZ で落ちない（`parseFmt`/`create`/`advanceFrom`/`nodeById` は arrow body 内の runtime call のみで定義位置非依存）。**parity guard（test_nodes L62）の `model` 追加は R1 で既済**ゆえ test 側変更ゼロ＝hub.mjs 1 file の純リファクタ。⏭ skip：**test の RUN set は hub.mjs の `RUN` 表を introspect せず ui2.html COMP に対する静的 set**（drift signal は palette↔runner で別経路ゆえ B6 で表化しても guard の正本性は不変・brittle な JS パースを避ける意図的設計）。検証＝`node test_nodes.mjs`（parity 11 kind＋全 kind E2E＋model plain/consensus＋旧 kind 後方互換 green）／全スイート 11本 green（`test_r0_snapshot`＝baseline byte 一致で決定論性確認・`test_reliable`＝crash recovery）／hub 実起動 `--vendor stub --port 7799` → `/api/health` `{"ok":true,"scheduler":true}`。**これで runner-core チェーン B4→B5→B6 完了**。残＝B7（trust dedup・`isCrossCompany` 再利用）／B8（HTTP route 表）／W1-4（n8n 視覚粒度・独立）。
...
**✅ 実装結果（2026-06-25）**：`evaluateEdgeFence(edge,value,sc,tc)→{text,removed,cross,never}` を trustPreview 直前に新設＝**純関数（trail/audit/save なし）**。`fenceEdge`（live）と `trustPreview`（dry-run）の重複3点（`edge.share.never` 抽出／`redact`／`isCrossCompany`〔B2 再利用〕）を集約。**audit は呼び側に残置**＝`fenceEdge` だけが `trail('redact',…)` を打ち、`trustPreview` は結果参照のみ＝**dry-run の read-only 性が構造で保証**（ヘルパに副作用が無いので破れない）。⚠ **ROADMAP 署名 `{text,removed,cross}` に `never` を1フィールド追加**＝trustPreview は fences ラベル `never:${never.join(',')}` に never 配列が要り、返さないと「never 抽出」重複が残るゆえ。`sc`/`tc` は**引数で受ける**（live＝`companyOf(run,…)`／dry-run＝`companyOfNode(node)` と会社の引き方が非対称ゆえヘルパ内で導出しない）。`redact` が `String(v??'')` で undefined 安全ゆえ dry-run は upstream 未確定（`up===undefined`）でも無条件にヘルパを呼び、`previewRemoved` を null に保つ判定（`up!==undefined?removed:null`）だけ呼び側に残す＝`knownUpstream:false`（上流不明）≠ 除去ゼロ`[]` の意味を保存。検証＝hub test 11本 green（`test_shenron`＝trust 純ユニット主・`test_nodes`＝全 kind E2E parity）＋**実起動 smoke**（`--vendor stub`・別 STATE_DIR）4点＝① live fenceEdge：secret(`[redacted:openai-key]`)＋never(`[redacted:never:…]`)を wire で除去し `trail(redact)` を audit に記録、② dry-run trustPreview：`previewRemoved` 正＋audit 件数 before==after で **read-only 実証**、③ dry-run 上流不明→`previewRemoved:null`（`[]` でなく）、④ cross-company（Acme→Globex）を `crossCompany:true`＋`fences:['…','cross-company']` で fence。**これで低リスク独立 B 群の B7 完了。残＝B8（HTTP route 表）／W1-4（n8n 視覚粒度・独立）。**
...
**✅ 実装結果（2026-06-25）**：`createServer` ハンドラ約420行の flat if 連鎖を `const ROUTES`（完全一致・キー `'METHOD path'`）＋`const RX`（regex 25本・登録順 first-match）＋`matchRx` に表化。**位置依存の2面ゲートを廃し、各 route が `auth` 列を宣言**＝`open`／`bearer`（/api 面・POST は旧ゲート同様 Bearer hint 付）／`admin`（唯一＝`auth/role`・旧 inline isAdmin を列に昇格）／`self`（`/mcp*` の OAuth challenge・`auth/me` の session cookie は handler 自前）。handler 署名は `(req,res,{u,p,j,m})` の大きな1つに統一＝**body は逐語移送**（u/j/m 名を保持し1bit も変えない）。⚠ **3つの faithful 保存**：①`POST /` の MCP は body 依存（`j.jsonrpc`）ゆえ表に入れず async 先頭に inline 残置、②`/.well-known`・`/oauth/authorize`・`/mcp/sse` は method 不問の `ANY` キーで同期相に保持（旧コードが method ガード無しで素通しだった挙動を保存）、③**未マッチ /api/* も旧面ゲートを catch-all で適用**（unknown GET→401・unknown POST→401／token 有なら 405・404）＝unknown route の認証応答も不変。検証＝**新 `test_routes.mjs`（B8-auth-map §5 を符号化＝公開例外/面ゲート/admin403/oauth公開/mcp自前401/OPTIONS204/regex dispatch）を旧コードで先に green 化** → 表化後も同 green ＝認証境界 byte 等価の前後証明／全12本 green（既存11＋routes）／**実機 boot smoke**（`--vendor stub`・別 STATE_DIR）で health・readiness・静的HTML・state・pubkey(PEM content-type)・OPTIONS(204)・goals/:id regex・/mcp tools/list(200)・unknown(404) を確認／**機械完全性照合**＝旧 exact-path 78本の欠落0・regex 25=25 で route 漏れゼロ。MCP-first 北極星（tools/list・mcpDispatch・skill）挙動不変。**これで低リスク独立 B 群＝B1・B2・B3・B7・B8 完了。残＝W1-4（n8n 視覚粒度・独立）／PC3（needs/blocker/cost ブリーフ）。**
...
**✅ 実装結果（2026-06-25・`canvas-n8n-s1-ports`）**：ui2.html の**3編集・純描画**（6挿入3削除・1 file）。①`:root`（L10）に `--aiwire:#a371f7;`、②`typeColor` 直後（L596付近）に `const AI_AUX = new Set(['model','parser']);`＋`const isAiEdge = (e)=>AI_AUX.has(comp(e.source)?.kind)||AI_AUX.has(comp(e.target)?.kind);`、③`drawLinks`（L859-862）に `const ai = !fenced && !br && isAiEdge(e);` を追加し `col` に `:ai?'var(--aiwire)'`、dasharray に `:ai?' stroke-dasharray="5 5"'` を挿入。⚠ **Pass2 で spec の前提崩れを捕捉**＝spec L632 の `AI_AUX={languagemodel,structured,parser,consensus}` は R1 前の集合。R1 で `KIND_ALIAS` が旧 LLM kind を `model`+`config.mode` に昇格済（live kind＝`input/model/output/parser/router/workflow/langflow`＋agent/trigger/mcp/note）ゆえ **`{model,parser}` に修正**（L671 の R1 後集合・W4 と一致＝同じ edge を円＋◆に昇格する視覚言語の連続性を保つ）。アクセサは spec の `nodeOf` でなく既存 `comp()` を採用＝`drawLinks` の `br` 行と同一イディオム（trigger/mcp/note と agent を undefined→false で弾く・AI_AUX に該当しないので結果同値）。**既存挙動の不変性は構造保証**＝`ai = !fenced && !br && isAiEdge` ゆえ非AI/非router/非fenced edge は `col`/`dasharray` が1bit も変わらず、fenced(amber `6 3`)＞router分岐(青/灰・実線)＞AI(`--aiwire` `5 5`)＞データ(typeColor・実線)の優先順位を `!br` ガードで担保。検証＝全 13/13 green（`test_all.mjs` 単独・`test_canvas` 含む／※並列＋二重起動だと test_all の子と衝突して偽 fail＝単独実行が正本）／hub 実起動（`--vendor stub`・別 STATE_DIR）health green ＋ live `/ui2` で4マーカー配信確認。⏭ **ブラウザ実描画は未実施**（data-path 確認のみ）＝SVG 文字列構築は決定的でマーカー配信＋`test_canvas` で失敗面は網羅、chrome 拡張接続時に `model→output` を1本引いて破線紫を目視（次トリガ）。**これで W1 完了。残＝W2（アイコン色タイル＋ノード形）／W3（線先 + 追加）／W4（AI sub-node 円＋◆・最大）／QW。**
...
**✅ 実装結果（2026-06-25・`canvas-n8n-s1-ports`）＝W2 を「種別を即認識（純表示）」に scope 確定**：W2 spec 7要素のうち**①アイコンタイル＋per-kind accent ②アイコン拡大 ③trigger D字を出荷**。**⑥router OUT 2-dot＋⑦branch スレッド配線は W2b に分離**＝後者は `edge.branch` を書き込む**配線データ操作**（router 分岐実行に絡む別 concern）ゆえ純表示 Wave と混ぜず WIP=1・revertable を維持。実装＝ui2.html の CSS 1ブロック＋renderNode の4分岐マークアップ（23挿入4削除・1 file）。CSS：`.icontile`（26×26 角丸・`svg{width:18px}` で 14→18 拡大＝NI 呼び出し不変で Haiku-safe）＋`.node.{agent,trigger,mcp}.icontile` のカテゴリ色＋`.node.comp[data-kind="…"] .icontile` の per-kind accent（**R1 後 live kind＝input/output=teal・model=紫・parser=緑・router=橙・workflow=indigo・langflow=pink**・unset は `.node.comp .icontile` default 紫）＋`.node.trigger{border-top-left-radius:24px;…}` の D字。マークアップ：agent/trigger/mcp/comp の `.name` 内アイコンを `<span class="icontile">…</span>` でラップ＋comp 分岐に `n.dataset.kind=kind`。⚠ **Pass2 で spec の accent マップ（R1 前 kind 列挙）を R1 後へ読み替え**（W1 と同型の衝突）＋**spec の `renderNodes 各ループ` は R0 で `renderNode(node,canvas)` 単一分岐器に統合済ゆえそこを編集**。**icon は `currentColor` 駆動**（NI svg が `stroke="currentColor"`）ゆえ tile の `color:` で再着色が効く。**データ不変の機械証明**＝`test_r0_snapshot` が drift 13キー全て `card:*`（描画HTML）のみで **`buildFlow`／`buildFlow_afterUndo` は byte 不変**＝保存 JSON ゼロ変更を golden test が分割証明 → 意図的視覚変更につき `--write` で baseline 再生成（`r0_baseline.json` 同 commit）。検証＝全14/14 green（`test_runner.mjs` が別 claude 追加で 13→14・`test_canvas`＋再生成 `test_r0_snapshot` 含む）／hub 実起動 health green＋live `/ui2` で5マーカー配信（icontile×4・data-kind setter・per-kind CSS・D字）。⏭ **ブラウザ実描画は未実施**（data-path 確認のみ）＝色タイルの視認性は主観品質ゆえ次に chrome 接続時に palette 全種配置で目視（次トリガ）。**これで W2 完了。残＝W2b（router 2-dot＋branch スレッド）／W3（線先 +）／W4（AI sub-node 円＋◆）／QW。**
> **W2b（旧 W2 の⑥⑦・配線操作）— scope-split で新設**：router OUT を then/else の 2-dot 化（CSS `.port.out.then{top:28%}`/`.else{top:72%}`＝endpointPos の 0.28/0.72 レーンと一致）＋`startWire(e,src,branch)`/`tryConnect(src,tgt,branch)` に branch を通し dot 選択で `edge.branch` を保存。**トリガ＝router の else 枝を UI から引きたくなった時**（現状 else は inspector 経由 or default 'then' のみ）。データ操作ゆえ単独 Wave で test（branch 保存の round-trip）付きで実装。
...
- **🗓 セッション状況（2026-06-25）**：**R0＋R1 実装完了**。R0＝ui2.html 4配列→`NODES[]` 統一（`ab6ffd3`＋`d5731e7`）。R1＝`model` ノード統合（旧 prompt/languagemodel/structured/consensus → 単一 `model`＋`config.mode`・kind 10→6）。frontend(ui2.html)＝COMP に `model`／旧4削除／input・output `hidden`／`KIND_ALIAS` を loadFlow 適用／`MODEL_MODE_FIELDS` で mode 別フィールド。backend(hub.mjs)＝`fireNode` に `model` dispatch（**B6 最小版を同時化**・旧 if 温存）＋PORTS。**INSPECT/SPEC/CARD テーブル化は R1 でも不要**と判明（R0 で全経路 COMP 駆動済み）。Langflow import は alias 任せで未変更。検証＝test_nodes parity 11 kind＋model(plain/consensus)＋旧 kind 後方互換 green／test_r0_snapshot baseline 再生成＋alias assert green。**R2 着手済（同セッション3 Wave・2026-06-25）**：B1（`fe6852b`・saveWorkflow の dead steps[] 撤去＝server.mjs 3箇所の無ガード参照を Pass2 で捕捉）＋B-fix（`168c500`・stdio run_automation/fire_event を hub runFlow に一本化＝**本番 scheduler/remote は元々正常・stdio surface 限定バグ**）＋B2（`d76ced9`・filterTriggers/isCrossCompany 集約・genId は自明な1行ゆえ skip）。**B3（`21dee59`）＝vendor/model 解決を resolveVendor() に集約**（consensus 逆契約・生成系 `||'claude'`・judge route 直は対象外で残置）。**B4＝handoff `kind` 統一**（4生成サイトに `kind` 追加・型判別を純関数 `hkind()`〔kind 優先 + marker fallback〕に集約・migration shim は読取専用 fallback に変更／recovery を marker 有無の脆い判定から解放）。**残＝B5-8**（B5→B6 runner-core〔B5=`createInternalHandoff` factory／B6=`fireNode` dispatch 表・`kind` 前提〕／B7 trust dedup〔B2 `isCrossCompany` 再利用〕／B8 HTTP route 表）か **W1〜W4**（n8n 視覚粒度・独立）。
...
##### ✅ PC1 — Readiness 可視化（計画できる状態か）— 出荷 `e222236`・2026-06-25
...
**依存**：PC0 相補。
**✅ 実装結果（`e222236`・61挿入2削除・6ファイル）**：`hub.mjs` `plannerReadiness()`（availableSummary 隣・vendor/`ANTHROPIC_API_KEY`/`claude`・`codex` CLI で `model` 判定）＋ `GET /api/shenron/readiness`（doctor 直後・**認証ゲート前＝無認証**）／`tools.mjs` `shenron_readiness`（TOOLS 無タグ＝両 surface＋PROXY）／`server.mjs` stdio `case`（hub proxy）／`shenron.html` WISH 画面バッジ＋`ui2.html` topbar バッジ（`#ckptBadge` 隣）／`test_nodes.mjs` に readiness HTTP e2e。検証 green＝全 11 test（surface guard **stdio 79/remote 72**・readiness e2e=`--vendor stub`→`model:false`+fix）＋**実起動 2 ケース**（stub→`model:false` ／ claude CLI 検出・キー無し→`model:true`）。
- **予測とのズレ5点（後続 PC で再利用）**：① **ESM で `require` 不可** — draft の `require('child_process').spawnSync` は hub.mjs(ESM) で crash。既存 `import {spawn}` を `{spawn, spawnSync}` に拡張（L23）。② **`spawnSync` はイベントループを同期ブロック** — badge poll（ui2 refresh 1.5s）毎に再 spawn すると hub が最大3s フリーズ。`_cliProbe` を **per-process memo**（CLI 在否はプロセス内不変）にして初回1回だけ probe＝draft に無い必須追加。③ **server.mjs は PROXY を import せず** callTool の**明示 `case` で dispatch**（`hub_doctor` 同型）。draft は tools.mjs だけ記載だったが stdio surface に `case 'shenron_readiness'` 必須＝surface guard test(`test_shenron` L462-465 が `case '…':` を grep)が強制。④ **行シフト**（PC0 同様）：availableSummary 予測 L1269→実 **L1291**・route 予測 L1437(health 隣)→実 **L1483**(doctor 直後)・planFlow L1297→**L1308**。⑤ **shenron.html に横 topbar 無し**（左 icon rail のみ）→ draft「両 topbar」を変更し **WISH(計画する場所)にバッジ配置**＝🔴 を行動の接地点に出す方が honest（ui2 は実 topbar に配置）。⑥ **bonus**：`hasKey:false` でも claude CLI 検出で `model:true`＝**従量0 本人サブスクの堀**を readiness が直接実証。無認証ゆえログイン前から 🔴 を出せる（接続前から honest・秘密値は返さず boolean と件数のみ）。

##### ✅ PC2 — 相談の多ターン化（plan-mode 相当）＋要件 brief パネル — 出荷 `a5a9a93`・2026-06-25
...
**依存**：PC0。
**✅ 実装結果（`a5a9a93`・103挿入18削除・4ファイル）**：`shenron.mjs` に `mergeBrief(prev,{clarify,choices,blockers,assumptions})→{confirmed,open,assumptions,blockers}`（total・null安全）＋`briefText`（settled な confirmed/assumptions/blockers のみ整形）／`PROMPT`(L32)・`REFINE_PROMPT`(L271) に `brief` 引数を足し冒頭注入／`plan()`(L313-321) が `context.brief` を briefText 注入し clarify 返却に `mergeBrief` で蓄積 brief を載せる／`renderPlan` clarify(L153) に「― これまでに確定 ―」再掲／`PROMPT` clarify 契約に `"assumptions":[…]` を追加。`shenron.html` `submitClarify(deepen)`(L719) を `{question,answer}` pair＋`context:{choices,brief}` round-trip 化＋要件パネル（confirmed/assumptions・blockers は既存赤枠と相補）＋「もっと詰める」（deepen=空 choices でも brief で深掘り）。`tools.mjs` plan_flow description に brief round-trip を明記（MCP client も多ターン）。`test_shenron` に2往復蓄積テスト。検証＝hub test **11/11 green**＋hub 実起動 smoke 3点（① brief 入り context が HTTP→planFlow→shenronPlan 全スタック受理＝stub で PC0 unavailable・500なし ② PC1 readiness `model:false` 無回帰 ③ import/構文無破壊）。
- **予測とのズレ6点（後続 PC で再利用）**：① **配管は既に通っていた**＝`context` は HTTP route→`planFlow`→`shenronPlan`、さらに MCP stdio(`server.mjs:164`)/remote(`mcpDispatch` hub.mjs:1366) すべてで **opaque 転送済み**。`planFlow` の clarify 返却も `...ir` spread 済みゆえ **hub.mjs・server.mjs は変更ゼロ**で `brief` が全 surface を流れた（予測の (b) は当たりだが配管側作業は不要だった）。② **本丸は UI 側の回答蒸発**（予測に無し）＝旧 `submitClarify` は `Object.values(clarifyChoices)`＝**選択肢テキストのみ**を bare で送り2回目で `clarifyChoices={}` リセット→2回目 clarify で1回目の回答が消え planner にどの質問の答えか不明。`{question,answer}` pair 化で同時修正（brief 蓄積の前提）。③ **JS default 引数の罠**＝`mergeBrief(prev,{choices=[]})` の default は **`undefined` にしか効かず**、呼び出しが `context.choices`(明示 null)を渡すと `[...null]` で TypeError→clarify return を囲む try/catch に飲まれ heuristic 偽 plan に劣化（mode 消失）。helper を total（`...(choices||[])`）にして解消＝**「分岐を try の外に出す」より helper を null 安全化する方が副作用が小さく、context を渡す後続 PC 全 helper に効く**。④ **assumptions だけ planner emit 必須**＝confirmed/open/blockers は `mergeBrief` の機械蓄積で済むが4フィールド目 assumptions は planner が出さねば dead→clarify JSON 契約に1フィールド追加（PROMPT 最小侵襲）。`briefText` は **open を除外**（今 choices で解決中＝二重注入回避）。⑤ **行シフト**（PC0/PC1 同様）：clarify 返却 予測 L288-289→実 **L318-321**・clarify UI 予測 L154-189→実 **L160-216**（PC1 挿入分）・submitClarify→実 **L719-737**。⑥ **検証境界の正直記録**＝完全な多ターンループ（clarify→plan）は**実モデルが要る**（stub は PC0 unavailable ゆえ HTTP で clarify を踏めない）→蓄積は `test_shenron` の run 注入で**決定的に単体検証**し、HTTP は配管受理 smoke に留めた。北極星＝tools.mjs 同期で **cockpit-only な穴なし**（MCP client も brief round-trip で多ターン）。
...
**✅実装済み（`e840690`+`ae5cb6f`・push 済＝並列 claude の W1 `1bd1748` に同梱で origin へ）**：`runner.mjs` `runVendorAsync` に `vendor==='mock'` 分岐（env `SHENRON_MOCK_PLANNER` の JSON 配列を 1 コールずつ shift・文字列=生出力／オブジェクト=stringify）＋ `test_all.mjs`（glob 子プロセス集計）＋ `test_planflow_http.mjs`（clarify→clarify→clarify(再回答)→plan の 4 ターン HTTP）＋ **#2 dedup-by-question**（`mergeBrief` の confirmed を質問キーで最新上書き＝`briefItems(_, byQuestion=true)`・assumptions/blockers は全文 dedup のまま不変）。**`node prototype/hub/test_all.mjs` で 13/13 green**（既存 12＝予測 11 より +1・`test_routes.mjs` が R2 B8 で既存化していた／glob runner ゆえ count 非依存で自動追従）。手動実機起動（`--vendor mock --port 8802`）で health/readiness/plan 3 連も確認。
> **予測ズレ（実装で判明）**：① scenario ③ に **`save:true` が必須**（route は `save: j.save` を渡し `workflowId` は save 真時のみ・hub.mjs:1358）→ test で補完。② 空 queue は **guard clause で生 stub を返す**よう変更（ROADMAP の literal は `JSON.stringify(stubOut)` で stub をクオート二重化し `isStubFail` の `^\[stub\]` を外す＝unavailable へ落ちなかった／簡潔化も兼ねて修正）。③ readiness は mock vendor で `model:false`（mock は test seam ゆえ honest・flow 実行は EXEC_VENDOR='mock' で seam を貫通するので無関係）。④ #2 の回帰番兵は test_planflow_http に ②'（Q1 を別回答で再送→confirmed が最新だけ）として同梱＝client 端の T2 を待たず server 端で先に固定。
...
## Wave N 計画 — n8n 採用バックログ（moat-first・設計のみ📋・build＋node 統合・2026-06-26）

**出自**：`docs/n8n/build`（n8n 公式 Build doc・10,931 行）完全精読 → honest loss 評価 → whitespace（勝てる空白）判定を経て user が **16 件の採用を確定**（N1〜N11＋🟡defer）。**2026-06-26、`docs/n8n/node`（ノード・リファレンス 10,610 行）精読由来の機能拡張（N12〜N19）を本 backlog に完全マージ**（旧『Wave Node-n8n／NN1〜NN10』案は重複排除のため吸収・破棄。完全重複は既存 N に統合＝Wait→N9／Return Intermediate Steps→N3／$fromAI→N5／Stop&Error→N10）。
...
### N1 — gen-eval（生成品質を測れるようにする＝堀の生死）✅ 実装済 `dc52ddb`（2026-06-26）
...
**✅ 実装結果（`dc52ddb`・2026-06-26）**：spec 通り 8 step 完遂。`runGenEval`（shenron.mjs・純粋関数）＋`doGenEval`（hub.mjs・POST route と mcpDispatch の単一ソース＝重複ゼロ）＋POST/GET `/api/shenron/gen-eval`＋`genEvalHistory` ring(cap50)＋MCP `run_gen_eval`/`list_gen_eval`（stdio+remote・U-1 surface guard 通過）＋`gen-cases.json` 5件（stdlib 決定論3+network2・$0・無 credential）＋`test_shenron.mjs` 単体5本。**verify**：`test_shenron`/`test_nodes` green・hub 実起動 `/api/health` 200・POST→`{cases:5,convergeRate:0,passRate:0}`（fake claude で vendor 不在→fail-fast＝「測れた(0%)」spec 予測通り）・GET history 1→2 行で回帰差分計算可。**spec とのズレ**：rec の時刻は spec「ts」でなく checkResults 同型の `at`(ISO) を採用（sibling ring と一貫）。**defer（[[feedback_skip_record]]）**：①cockpit thin view＝MCP+GET 完備で北極星不変ゆえ次 sub-wave・**トリガ＝N6(plan real-time UX) か settings 系 Wave 着手時に同梱**／②judge case＝dataset append のみ・**トリガ＝assert seed の passRate が安定後に追加**。**⚠ 並列 claude**：本 Wave 中に同一 working tree で別 claude が hub.mjs(`hasKey` 削除)・test_routes.mjs・ROADMAP(L64)・ui2/r0 を並行編集。safe-commit で hunk 隔離し N1 の6ファイルのみ commit（foreign 変更は未コミットで温存）。
...
- **N9 — #35 Wait node＋HITL 承認の一般化（durable resume・cross-company moat）**（🟢・node 由来 NN1 統合）：フロー途中で待機/人間 checkpoint＋**実行を pause し DB 退避→resume**（After Time Interval／At Specified Time／**On Webhook Call**〔`$execution.resumeUrl`・Basic/Header/**JWT** auth・IP whitelist〕／On Form Submitted）＋Limit Wait Time。**承認フェンス一般化**＝応答型（Approval inline／Free Text／Custom Form）・承認チャネル routing（cockpit/Slack/Telegram/メール）・timeout 自動。`$execution.resumeUrl` は **cross-company A2A handoff の核**（相手の応答を握りっぱなしにせず durable に待つ＝北極星直結）・承認イベントを hash-chain audit に刻む（n8n/Langflow に無い合体）。アンカー `ui2.html` `COMP` L533（新 kind `wait`）／`hub.mjs` `fireNode` L564／`firePromptNode` L594（`touch(h,'awaiting_approval')` L694 が手本）／`KIND_ALIAS` で後方互換。MCP `resume_execution`/`respond_approval`・hub `/api/resume/:token`。リスク中（canvas+backend+MCP 3点同時 [[feedback_ui_sync]]）。
...
**tier 4＝node 由来の flow control・データ変換・成熟度（`docs/n8n/node` 10,610 行精読・2026-06-26 完全マージ・大きな部品で 8 本に統合）**
> 旧『Wave Node-n8n／NN』案の net-new のみ（完全重複は N3/N5/N9/N10 に統合済）。実装は WIP=1・着手前に個別 3-pass。アンカーは既出の確認済み行を再利用。

- **N12 — Switch 化（N 出力条件分岐・If 内包・未着手 W2b 吸収）**（🟢・canvas+backend）：現 `router`（then/else）→ **N 出力**。Rules（出力毎マッチ・**Rename Output**）＋Expression（出力 index を式算出）＋**Fallback Output（None/Extra/Output 0）**＋型別演算子（String/Number/Date/Boolean/Array/Object＝Filter/Switch 共通）＋Send-to-all-matching。If＝2 出力特殊形（既存 router）。アンカー `ui2.html` `COMP` L533（router kind）／`hub.mjs` `advanceFrom` L831・`markDead`／`canConnect`/`matchType`/`drawLinks`。**未着手 W2b（router 2-dot＋branch 配線）を吸収**＝2-dot を N-dot に一般化（`startWire(branch)`/`tryConnect(branch)`/`edge.branch` を N 本へ）・別 Wave で作らない。W4（◆）とは別レイヤー。⚠ legacy「If+Merge 両枝実行（v1.0 撤去）」は再現しない。リスク中（canvas+backend 同時 [[feedback_ui_sync]]）。
- **N13 — item 配列基盤＋変換ノード群**（🟢・大→ N13a/N13b 分割推奨）：(a) **item 配列処理**（暗黙ループ＝1 ノードが複数 item を順処理）基盤＝N13a。(b) **Merge**（全入力待ちバリア＝Append／Combine by Matching Fields〔inner/left/right/outer〕／Position／All Combinations／**SQL Query**／Choose Branch・Clash Handling）・**Aggregate**（多→1）・**Split Out**（1→多）・**Summarize**（ピボット集計＋group by）・**Limit**（先頭/末尾 N）・**Sort**（Simple/Random/Code）・**Remove Duplicates watermark**（現入力内＋**過去実行またぎ**: Value Is New／Higher／Date Later・Scope Node/Workflow・History Size）＝N13b。アンカー `hub.mjs` `fireNode` L564／`runFlow` L416・Merge は §6.5 既存 `\n\n` 連結を join 戦略へ拡張。神龍差分＝単一テキスト中心で複数件処理が弱い・Merge join は連結より桁違い・Remove Duplicates は scheduler 二重処理を実害回避。**N13a が N14 解析 mode の前提**。リスク中。
- **N14 — Model 本番化＋解析 mode**（🟢・R1 `model`/agent 上）：(a) 本番化＝**Max Iterations**（暴走上限・既定10）・Tracing Metadata・**temperature=0**（決定性）・**Auto-Fixing 出力パーサ**（schema parse 失敗を LLM に投げ直し修復）・**Schema 3-way**（Attribute Descriptions／JSON Example／JSON Schema）。(b) 解析 mode＝**Information Extractor**（構造抽出）・**Text Classifier**（分類・複数可・'Other' 枝＝N12 整合）・**Sentiment**（感情・強度/信頼度・temp0）・**Summarization Chain**（Map Reduce/Refine/Stuff・chunking）。kind を増やさず `model` の mode を増やす（大きな部品）。アンカー `shenron.mjs` `genComponent` L418／`evalExpect` L476（再利用）／`hub.mjs` `firePromptNode` L594。**Return Intermediate Steps＝N3／$fromAI＝N5 を前提**（重複回避）。temp0 は PROJECT.md 非決定性対処・Auto-Fixing は Wave R（自己修復）synergy。**W4（◆ AI sub-node 接続）の視覚 consumer**。リスク中。
- **N15 — Authoring UX（inspector 横断）**（🟢・R1 NDV 上）：**Resource→Operation 二段**（mcp server→tool／`model` mode）・各 field の **Fixed↔Expression トグル＋即時評価プレビュー**・データ操作の **Dry Run**（実行せず before/after＝trust dry-run の data 版）・Mapping 手動/自動。アンカー `ui2.html` `inspNode`／`COMP` L533。「自由入力/生 JSON→判断できる UI」方針の本丸。リスク低〜中。
- **N16 — Schedule Trigger guided UX**（🟢・既存 scheduler 上）：生 cron を書かせない **guided builder**（Sec/Min/Hour/Day/Week/Month＋Trigger at Hour/Minute/Weekday/Day-of-Month＝「毎週月曜9時」を選ぶ）＋Custom Cron（6 フィールド・秒任意）＋複数 Trigger Rules＋timezone。アンカー `hub.mjs` `tickScheduler` L1136／`lastDue` L1142／`SCHED_FILE` L1120（GUI 選択→cron 文字列→既存 scheduler 発火）。リスク低。
- **N17 — MCP surface 拡充**（🟢・MCP control plane 拡張）：**MCP Client** に OAuth2／Multiple Headers auth／tool 自動列挙 UI／timeout。**MCP Server Trigger 相当**に workflow（sub-flow）を MCP tool 公開／**Test・Production URL 分離**（test=データ表示/prod=audit 経由）／path カスタム。アンカー `server.mjs`／`tools.mjs`／`mcp-client.mjs` `callMcpTool` L21／`hub.mjs` `runMcp` L704。claude.ai connector 検証（残課題）にも効く。**$fromAI＝N5（引数層）ゆえ非重複（本件は接続層）**。リスク中。
- **N18 — governance（成熟度・安全）**（🟢・WIP=1 で sub 分割）：(a) **typeVersion 版管理**（component/agent に版＋Deprecated/Removed・旧版は動かす・移行先明示＝§12 KIND_ALIAS の上位）。(b) **verified/未審査 → passport 連動**（未審査 component は net:none/external_send:deny で sandbox＝skill marketplace 安全土台・n8n は警告のみ→神龍は機械的に封じ込め）。(c) **Retry On Fail／Batching／Optimize Response**（rate limit・トークン節約）。(d) **Code/生成 component 安全裏取り**＝出力形 validate（`[{json:{}}]`）／credential 非アクセス／module allowlist（Wave N-1 整合）。**Stop And Error 明示ノードは既存 N10（#38 error workflow）に統合**（別実装しない）。アンカー `mcp-client.mjs` `SECRET_RE` L16・`safeEnv` L17／`shenron.mjs` `neededCredentials` L395／`hub.mjs` `runFlow` L416（failed 経路）。リスク中。
- **N19 — utility ノード bundle（随時・1 ノード=1 small commit）**（🟢）：必要時に大きな部品で＝**JWT**（sign/verify）・**Markdown**（MD↔HTML）・**XML**・**Convert to File**（CSV/PDF/XLSX）・**Date & Time**・**HTML**（テンプレ/抽出）・**Read/Write Files**・**SSH**・**Send Email**（SMTP）・**RSS**・**Compare Datasets**・**Edit Image**・**Execute Command**・**FTP**・**Git**・**Debug Helper**（Throw Error/OOM/乱数＝テスト道具）。メタ＝n8n API ノードから **audit 一括生成・credential schema** の語彙のみ借用（神龍は MCP で先行）。依存なし。
