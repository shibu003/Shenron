...
- **Langflow テンプレ分析（2026-06-16・user 提供 `content_cascade_flow`）の収穫＝次の借り先**: ① **field-widget 語彙拡張**＝component の `fields` 型に `bool`/`secret`/`dropdown(options)` を追加（Langflow の SecretStr/Bool/Dropdown 相当・③a inspector に効く・安価で高価値）。② **note ノード**（markdown 注釈カード・`backgroundColor`）＝安価・「flow を説明する」UX に。③ **multi-handle typed ports**（1 ノードに複数 named 入出力・型は `output_types ∩ inputTypes`）＝**大改修（React Flow 必要）→ 温存**（docs/11 §4 既出の繰延）。④ node morphing（`real_time_refresh` で field/outputs 可変）＝中。⑤ typed handle color は `typeColor()` で実装済。
- **UI 方針（user・2026-06-16「UI は真似すればいい」）**: cockpit の見た目を **Langflow 寄りに寄せる**（pattern 流用・**コード literal copy は禁止**＝philosophy #1）。次の見た目 Wave で node カード/handle/palette の見せ方を Langflow 風に。
