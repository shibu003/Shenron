...
- **UI 方針（user・2026-06-16「UI は真似すればいい」）= Langflow 寄せ（pattern 流用・コード copy 禁止＝philosophy #1）**: **✅ (a) node カード restyle（`05bb8a5`）**＝ヘッダ帯＋区切り線＋角丸14＋カテゴリ色 tint（trigger amber/mcp green/comp purple）＋幅統一。**残り＝palette 行/handle/アイコン chip のさらなる寄せ**（次の見た目 Wave・headless Chrome `--screenshot` で before/after 検証可＝`/Applications/Google Chrome.app/...`）。
