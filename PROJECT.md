...
7. **▶ 拡張 F/G（次の一手・user 要望・`docs/11 §2.5`）**: **F** integrations/⚙settings（MCP 接続＋on/off・**autorun on/off**・**share=渡す/絶対渡さない情報の切り分け**）→ **G** `kind:"mcp"` tool ノード＋executor 実呼び出し＝**「submit 後に実際に外部へ送信」**（approval フェンス＋share 境界を通してから送信）。
